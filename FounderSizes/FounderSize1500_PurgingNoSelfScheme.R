#Repeat Simulation in April, 2019 for StandardScheme=NoSelf1500FounderSize
library(rrBLUP)
library(snowfall)
...
s <- Sys.time()
###############################################################
# Modifications for noSelf indicated with #mns
library(BreedingSchemeLanguage)
...
selfFertilize(nProgeny=1, popID=5) # popID 6 #mns
phenotype(plotType="Seedling", popID=6)
select(nSelect=1,random=F, type="WithinFamily", popID=6) # popID 7 #mns
...
selfFertilize(nProgeny=1, popID=9) # popID 10 #mns
phenotype(plotType="Seedling", popID=10)
select(nSelect=1,random=F, type="WithinFamily", popID=10) # popID 11 #mns
cross(nProgeny=nProgeny, popID=9) # popID 12 #mns
genotype(popID=12)
...
selfFertilize(nProgeny=1, popID=14) # popID 15 #mns
phenotype(plotType="Seedling", popID=15)
select(nSelect=1,random=F, type="WithinFamily", popID=15) # popID 16 #mns
...
selfFertilize(nProgeny=1, popID=20) # popID 21 #mns
phenotype(plotType="Seedling", popID=21)
select(nSelect=1,random=F, type="WithinFamily", popID=21) # popID 22 #mns
cross(nProgeny=finalPop, popID=20) # popID 23
print(paste("Cycle 5 Created", Sys.time() - s))

outputResults(summarize=FALSE, saveDataFileName=paste(baseName, "FounderSize1500_NoSelfAllOut", sep=""))
pdf(paste(baseName, "FounderSize1500_NoSelfPlot.pdf", sep=""))
plotData(addDataFileName=paste(baseName, "FounderSize1500_NoSelfPlotOut", sep=""), popID=list(0:1, c(4:5,13,19), c(8:9,18), c(12,14), c(17,20), 23))
dev.off()
