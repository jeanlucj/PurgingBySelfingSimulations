#Repeat Simulation in April, 2019 for StandardScheme=SelfMean
library(rrBLUP)
library(snowfall)
...
outputResults(summarize=FALSE, saveDataFileName=paste(baseName, "Standard_MeanSelfAllOut", sep=""))
pdf(paste(baseName, "Standard_MeanSelfPlot.pdf", sep=""))
plotData(addDataFileName=paste(baseName, "Standard_MeanSelfPlotOut", sep=""), popID=list(0:1, c(4:5,13,19), c(8:9,18), c(12,14), c(17,20), 23))
dev.off()
