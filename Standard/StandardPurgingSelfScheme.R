#Repeat Simulation in April, 2019 for StandardScheme=Self
library(rrBLUP)
library(snowfall)
...
outputResults(summarize=FALSE, saveDataFileName=paste(baseName, "Standard_SelfAllOut", sep=""))
pdf(paste(baseName, "Standard_SelfPlot.pdf", sep=""))
plotData(addDataFileName=paste(baseName, "Standard_SelfPlotOut", sep=""), popID=list(0:1, c(4:5,13,19), c(8:9,18), c(12,14), c(17,20), 23))
dev.off()
