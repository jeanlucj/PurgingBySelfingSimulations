#Repeat Simulation in April, 2019 for Additive model
library(rrBLUP)
library(snowfall)
...
sims <- lapply(sims, function (bsl){bsl$mapData$domModel <- "HetHom"; bsl$mapData$actionType <- rep(0, length(bsl$mapData$actionType)); return(bsl)})
...
outputResults(summarize=FALSE, saveDataFileName=paste(baseName, "AdditiveModelFounder1500SelfAllOut", sep=""))
pdf(paste(baseName, "AdditiveModelFounder1500SelfPlot.pdf", sep=""))
plotData(addDataFileName=paste(baseName, "AdditiveModelFounder1500SelfPlotOut", sep=""), popID=list(0:1, c(4:5,13,19), c(8:9,18), c(12,14), c(17,20), 23))
dev.off()
